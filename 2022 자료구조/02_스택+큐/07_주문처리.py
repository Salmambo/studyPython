...
class Queue:
    ...
    def pop(self) :
        ...
        '''
        if self.empty() == 0 : del self.myQueue[0]
...
def selectQueue(normalQueue, vipQueue, time, orders) :
    ...
    if vipIndex == -1 : return normalQueue
    if normalIndex == -1 : return vipQueue
    if time < orders[normalIndex].time and time < orders[vipIndex].time :
        if orders[vipIndex].time <= orders[normalIndex].time : return vipQueue
        else : return normalQueue
    if time >= orders[normalIndex].time and time < orders[vipIndex].time :
        return normalQueue
...
def processOrder(orders) :
    ...
    for i in range(len(orders)) :
        ...
        if orders[i].vip == 0 : normalQueue.push(i)
        else : vipQueue.push(i)
